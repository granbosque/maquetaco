...
export async function exportToEpub(config, options = {}) {
    ...
    if (sections.length > 0) {
        sections.forEach((section, i) => {
            const { content, sectionClasses } = sectionToChapterContent(section);
            epub.addChapter({
...
// Como el centrado vertical es problemático, si detectamos algunas clases, se envuelve en un div adicional
// Aun así, no es compatible con todos los lectores (kindle no funciona)
const VERTICAL_ALIGN_CLASSES = ['vcenter', 'vbottom'];

function sectionToChapterContent(section) {
    const needsVerticalWrapper = section.classes?.some(c => VERTICAL_ALIGN_CLASSES.includes(c));
    if (needsVerticalWrapper) {
        return {
            content: `<div class="vertical-align-wrapper"><section class="${section.classes.join(' ')}">${section.html}</section></div>`,
 ...
 * Reutiliza el wrapper genérico del tema (vertical-align-wrapper + vbottom)
 * para posicionar el contenido al final de la página; los estilos de .made-with-content
 * están en el CSS del tema EPUB.
 * @returns {string}
 */
function createBrandingHtml() {
    const content = `<svg width="48" height="27" viewBox="0 0 67 37" xmlns="http://www.w3.org/2000/svg" style="display: block; margin: 0 auto 0.5em auto;">
...
<p>Maquetado con Maquetaco</p>`;
    return `<div class="vertical-align-wrapper"><div class="vbottom made-with-content">${content}</div></div>`;
}
